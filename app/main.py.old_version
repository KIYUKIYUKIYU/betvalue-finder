# app/main.py — jp_to_pinnacle(CSV唯一の正)に準拠 /evaluate + /map + /ingest + SQLite保存
from fastapi import FastAPI, HTTPException
from pydantic import BaseModel, Field
from typing import List, Literal, Optional
from datetime import datetime, timezone
import math, json, sqlite3, os, threading

from app.converter import jp_to_pinnacle

app = FastAPI(title="BetValue Finder API", version="0.2.0")
...
def init_db():
    with _DB_LOCK:
        ...
        cur = con.cursor()
        # 入力/出力をまとめて保存する evaluations（なければ作る）
        cur.execute("""
...
def interp_prob(target_abs_line: float, lines: List[LineOdds]) -> float:
    ...
    for x, y in pts:
        if math.isclose(x, target_abs_line, rel_tol=1e-9, abs_tol=1e-9):
            return y
    ...
    if target_abs_line < pts[0][0]:
        ...
        if math.isclose(x1, x2):
            return y1
        slope = (y2 - y1) / (x2 - x1) if not math.isclose(x1, x2) else 0.0
...
@app.get("/")
def root():
    return {"status": "ok", "service": "betvalue-finder", "version": "0.2.0"}
...
def evaluate(req: EvaluateRequest):
    """日本式 → ピナクル値（CSV唯一の正）→ 公正オッズ推定 → エッジ判定 → DB保存"""
    try:
        pv = jp_to_pinnacle(req.jp_handicap)  # 例: '1半1' → 1.55
    except Exception as e:
        raise HTTPException(status_code=400, detail=f"jp_handicap が不正です: {e}")

    if req.lines_override and len(req.lines_override) >= 1:
        p_home = float(interp_prob(abs(pv), req.lines_override))
    else:
        p_home = 0.5
...
def ingest(req: IngestRequest):
    """任意JSONをDBに保管したい場合の簡易エンドポイント（監査・再計算の土台）。"""
    with _DB_LOCK:
        con = sqlite3.connect(DB_PATH)
        cur = con.cursor()
        cur.execute("""
