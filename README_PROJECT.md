...
### ✅ 実装完了
1. **両側EV評価システム** - フェイバリット/アンダードッグ両方のEV計算
2. **汎用入力パーサー（universal_parser.py）**
   - 形式A: `チーム名<ハンデ>`
   - 形式B: `チーム名06`（数字後ろ）
   - 形式C: `チーム名 - チーム名` + `チーム名 ハンデ`
   - 形式D: 時刻・リーグ区分のスキップ
   - 略称対応（Rソックス、Wソックス、Dバックス）

### 🔄 未統合
- universal_parser.pyをmain.pyに統合（テスト済み、統合待ち）

### 📌 次回作業
```bash
# 1. パーサー統合
cp app/main.py app/main.py.bak_universal
code app/main.py
# → parse_paste_text関数をuniversal_parser版に置き換え
